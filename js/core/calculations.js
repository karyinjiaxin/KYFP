...
var Calc = (function () {
  ...
  function gwa4WelcomeBonusPct(choice, annualPremium) {
    var tiers = (GWA4_RATES[choice] || GWA4_RATES['Choice 5']).welcomeBonusTiers;
  ...
  function gwa4PolicyFeePct(choice, policyYear) {
    var schedule = (GWA4_RATES[choice] || GWA4_RATES['Choice 5']).policyFeeSchedule;
  ...
  function gwa4SurrenderChargePct(choice, policyYear) {
    var arr = (GWA4_RATES[choice] || GWA4_RATES['Choice 5']).surrenderChargeByYear;
  ...
  function gwa4Projection(annualPremium, choice, expectedReturnPct, years, premiumYears, dividendYieldPct) {
    ...
    for (var year = 1; year <= years; year++) {
      var isPayingYear = year <= payYears;
      ...
      // Loyalty Bonus is paid ANNUALLY from the specified policy year
      // onward (confirmed against the product pack's own worked example:
      // year 10 = $300, year 11 skipped due to a withdrawal, year 12 =
      // $360 — an ongoing yearly payment, not a single one-time bonus)
      // and isn't tied to ongoing premium payment, so it continues even
      // after the premium-paying term ends.
      var loyaltyBonus = year >= plan.loyaltyBonusYear ? accountValue * (plan.loyaltyBonusPct / 100) : 0;
      // Premium Bonus IS tied to "receipt of each payment of Basic
  ...
  function investmentPlanProjection(plan, projectionAge) {
    ...
    for (var age = startAge; age <= endAge; age++) {
      ...
      var netReturn = expectedReturn - (feesActive ? fees : 0);

      // grow prior balance, add this year's premium (mid-year convention simplified to full-year add)
      accumulated = accumulated * (1 + netReturn) + yearPremium;

      if (age === startAge) accumulated += welcomeBonus;
      if (age === stopAge) accumulated += loyaltyBonus;
